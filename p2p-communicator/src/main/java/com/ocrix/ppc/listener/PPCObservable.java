...
package com.ocrix.ppc.listener;
 ...
 * 
 */
public class PPCObservable {
	private boolean changed = false;
	private Vector<MessageReceiver> obs;

	/** Construct an Observable with zero Observers. */

	public PPCObservable() {
		obs = new Vector<MessageReceiver>();
	}
	 ...
	 */
	public synchronized void addMessageReceiverListener(MessageReceiver o) {
		if (o == null)
	 ...
	 */
	public synchronized void deleteMessageReceiverListener(MessageReceiver o) {
		obs.removeElement(o);
	}
	...
	public void notifySubscribers(Object msg) {
		...
		synchronized (this) {
			 ...
			 * that: ) a newly-added Observer will miss a notification in
			 * progress ) a recently unregistered Observer will be wrongly
		...
		for (int i = arrLocal.length - 1; i >= 0; i--)
			((MessageReceiver) arrLocal[i]).onMessage(msg);
	}
	 ...
	 */
	public synchronized int countMessageReceiverListeners() {
		return obs.size();
	}
}
